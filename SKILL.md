...
- behavior:
  ...
  - defaults no-argument execution to replacing managed runtime or plugin-source drift, retains `--overwrite-runtime-drift` as the explicit equivalent for existing automation, and accepts `--check` for read-only drift detection
